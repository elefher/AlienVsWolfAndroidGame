...
public class SkeletonRunner extends FlyObject {

	int sizeDestroyAnim, DesptroyAnimrepeats = 0;
	int totalSpeed = 0;

	public SkeletonRunner(int centerX, int centerY, int power, int speedX, int moveSpeed) {
	...
	public void setFlyobjectAnim() {
		...
		int size = Assets.skeletonrun.size();
		for (int i = 0; i < size; i++) {
			flyObjectAnim.addFrame((Image) Assets.skeletonrun.get(i), 15);
		}
	}
	...
	public void setDestroyAnim() {
		...
		sizeDestroyAnim = Assets.explosionC.size();
		for (int i = 0; i < sizeDestroyAnim; i++) {
			destroyAnim.addFrame((Image) Assets.explosionC.get(i), 20);
		}
	}
