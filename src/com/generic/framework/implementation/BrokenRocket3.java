...
public class BrokenRocket3 extends FlyObject {

	int sizeDestroyAnim, DesptroyAnimrepeats = 0;
	int totalSpeed = 0;

	public BrokenRocket3(int centerX, int centerY, int power, int speedX,
			int moveSpeed) {
	...
	public void setFlyobjectAnim() {
		...
		int size = Assets.brokenRocket3.size();
		for (int i = 0; i < size; i++) {
			flyObjectAnim.addFrame((Image) Assets.brokenRocket3.get(i), 10);
		}
	}
	...
	public void setDestroyAnim() {
		...
		for (int i = 0; i < sizeDestroyAnim; i++) {
			destroyAnim.addFrame((Image) Assets.explosionC.get(i), 100);
		}
	}
